...
class req(PersistentServerConnectionApplication):
    ...
    def handle(self, in_string):
        ...
        if form['a'] == "getnetwork":
            ...
            tasks = [
                *({'method': 'GET', 'url': f"https://admin.splunk.com/{server}/adminconfig/v2/access/{feature}/ipallowlists", 'ssl':verify, 'headers':[('Authorization',f"Bearer {token}")]} for feature in FEATURES),
                {'method': 'GET', 'url': f"https://admin.splunk.com/{server}/adminconfig/v2/access/outbound-ports", 'ssl':verify, 'headers':[('Authorization',f"Bearer {token}")]}
            ]
